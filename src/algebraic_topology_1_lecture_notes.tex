...
\begin{proof}
	...
	\begin{equation*}
		\tilde{H}_n(K(\Zn{n}, 1); \Z) \isom \begin{cases}
			\Zn{m} & n \text{ odd} \\
			0 	   & n \text{ even}
		\end{cases}
	\end{equation*}
...
\begin{theorem}\label{thm:protohurmodc}
	...
	\begin{enumerate}
		...
		\item $\pi_n(X; \Z) \in \catfont{C}$ for all $n \geq 1$.
	\end{enumerate}
\end{theorem}
Recall that a space $X$ is \strong{simple}\index{simple space} if it is path-connected, $\pi_1(X, *)$ is abelian and $\pi_1(X, *)$ acts trivially on $\pi_n(X, *)$ for all $n > 1$.
...
\begin{remark}
	...
	On the other hand, $X = S^1 \vee S^2$ shows that the theorem does not hold for all spaces, since $\pi_2(X, *) \isom H_2(\tilde{X}; \Z) \isom \dsum_{\Z} \Z$ where $\tilde{X}$ is the universal covering of $X$.
	% TODO: picture?
\end{remark}
\begin{lemma}
	Let $\catfont{C}$ be a Serre class satisfying the tensor axiom, $F \to Y \to X$ a fibre sequence of path-connected spaces with $\pi_1(X, *)$ acting trivially on $H_*(F; \Z)$ (i.e. the local system $H_*(F_{-}; \Z))$ is isomorhic to the trivial one).
	If two out of $F$, $Y$, and $X$ have $H_n({{-}}; \Z) \in \catfont{C}$ for all $n \geq 1$, then so does the third.
\end{lemma}
\begin{proof}
	We abbreivate $H_*({{-}}; \Z)$ to $H_*$ and distinguish the following cases:
	\begin{enumerate}
