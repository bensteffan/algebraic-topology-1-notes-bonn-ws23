...
Moreover, the set of stable cohomology operations $\{H^k({{-}}; A) \to H^{k + *}({{-}}; A) \mid k \in \Z\}$ forms a graded ring under composition.
This is generally not commutative.
\begin{proposition}\label{prp:cohopsbijections}
	We have bijections
	\begin{align*}
		\{\phi\colon H^k({{-}}; A) \to H^l({{-}}; B) \text{ cohomology operation}\} &\xto{\isom} H^l(K(A, k); B) \\
		\phi &\mapsto \phi(\iota_k)
	\end{align*}
	where $\iota_k \in H^k(K(A, k); A)$ is the fundamental class, and
	\begin{align*}
		\{H^*({{-}}; A) \to H^{* + n}({{-}}; B) \text{ stable cohomology operation}\} &\xto{\isom} \lim_{k \in \N} H^{k + n}(K(A, k); B)
	\end{align*}
...
\begin{proof}[Proof of proposition]
	...
	\begin{align*}
		\{H^{k + 1}({{-}}; A) \to H^{k + 1 + n}({{-}}; B) \text{ cohomology operation}\} &\to \{H^k({{-}}; A) \to H^{k + n}({{-}}; B) \text{ cohomology operation}\} \\
	...
	\begin{equation*}
		H^{k + 1}(K(A, k + 1); A) \isom \Hom(H_{k + 1}(K(A, k + 1); A), A) \isom \Hom(\pi_{k + 1} K(A, k + 1), A)
	\end{equation*}
	and likewise for $k$.
\end{proof}
\begin{corollary}
...
\begin{corollary}
	...
	\begin{enumerate}
		\item Let $k$ be odd and $l > 0$.
			Then
			\begin{equation*}
				\{H^k({{-}}; \Q) \to H^l({{-}}; \Q) \text{ cohomology operation}\} \isom \begin{cases}
					\Q 	& k = l \\
					0 	& \text{else}
				\end{cases}
			\end{equation*}
...
\begin{proposition}
	We have a bijection
	\begin{equation*}
		H^{2n}((E C_2)_{+} \wedge_{C_2} K(\F_2, n)^{\wedge 2}; \F_2) \xto{\isom} H^{2n}(K(\F_2, n)^{\wedge 2}; \F_2) \isom \F_2\{\iota_n \cup \iota_n\}
	\end{equation*}
\end{proposition}
...
Note that any group action on $\F_2$ is necessarily trivial, so that the local system $H^{2n}(K(\F_2, n)^{\times 2}, K(\F_2, n)^{\vee 2}; \F_2)$ must be constant.
...
% TODO picture?
For degree reasons there cannot be any nontrivial out of $E_2^{0, 2n}$, so we obtain a unique class $\alpha_n \in H^{2n}((E C_2)_{+} \wedge_{C_2} K(\F_2, n)^{\wedge 2}; \F_2)$ refining the cup square in $H^{2n}(K(\F_2, n)^{\wedge 2}; \F_2)$.
\begin{remark}
...
\begin{equation*}
	H^{2n}((E C_2)_+ \wedge_{C_2} K(\F_2, n)^{\wedge 2}; \F_2) \xto{\Delta^*} H^{2n}((B C_2)_+ \wedge K(\F_2, n); \F_2) \isom \bigdsum_{i = 0}^n H^i(B C_2; \F_2) \tensor \tilde{H}^{2n - i}(K(\F_2, n); \F_2)
\end{equation*}
...
\begin{definition}
	...
	\end{equation*}
\end{definition}
\begin{theorem}[Cartan-Serre]\index{Cartan-Serre theorem}
	For $n \geq 1$ we have 
	\begin{equation*}
...
\begin{proof}
	...
	\end{enumerate}
\end{proof}
\begin{theorem}[Borel's theorem]\index{Borel's theorem}
...
\begin{remark}
	...
	% TODO cite Hatcher Ch. 5 Thm. 5.34
\end{remark}
\begin{proof}
