...
The \emph{immersion conjecture}\index{immersion conjecture} states that if $N$ is any $m$-manifold, then $N$ immerses into $\R^{2m - \alpha(m)}$.
...
\begin{theorem}[{\cite[\thomcitestring]{thom_quelques_1954}}]\label{thm:thom}
	\leavevmode
	\begin{enumerate}
...
The key ingredient to computing these groups is the following theorem:
\begin{theorem}[Thom \cite{thom_quelques_1954}]
...
\end{remark}
% TODO find a working proof of this
...
Assuming for the time being that $M$ is connected, these monomials must all be zero as all Stiefel-Whitney numbers of $M$ are, so the \enquote{dual Stiefel-Whitney numbers}\index{Stiefel-Whitney numbers!dual} $\overline{\omega}_1^{n_1} \cdots \overline{\omega}_1^{n_k}[M]$ defined in analogy to the ordinary Stiefel-Whitney numbers are all zero.

Let now $f\colon M \to \Gr_{n - k}^\R$ be a classifying map for the normal bundle with respect to some embedding $M \incl \R^n$ (for instance the Gauss map).
...
\begin{align*}
	...
	[M] &\mapsto [\Phi_M]
\end{align*}
we conclude that $M$ is a boundary in $\R^n$.
...
\begin{enumerate}
	\item Use that $M$ is cobordant to a connected $M'$.
		For instance, $M$ is cobordant to $M_1 \csum \cdots \csum M_r$, the \emph{connected sum}\index{connected sum} of the $M_i$.
...
\begin{remark}
	...
	The $v_i$ are called the \strong{Wu classes}\index{Wu class} of $M$.
